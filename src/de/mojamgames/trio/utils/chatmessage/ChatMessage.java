package de.mojamgames.trio.utils.chatmessage;
...
public class ChatMessage {
    ...
    private Class<?> nmsChatSerializer = IChatBaseComponent.ChatSerializer.class;
    ...
    public ChatMessage(final String firstPartText) {
    ...
    public ChatMessage style(ChatColor... styles) {
    ...
    public ChatMessage file(final String path) {
        onClick("open_file", path);
        return this;
    }

    public ChatMessage link(final String url) {
    ...
    public ChatMessage achievementTooltip(final String name) {
        onHover("show_achievement", "achievement." + name);
        return this;
    }

    public ChatMessage achievementTooltip(final Achievement which) {
        try {
    ...
    public ChatMessage statisticTooltip(final Statistic which) {
    ...
    public ChatMessage statisticTooltip(final Statistic which, Material item) {
        Type type = which.getType();
        if (type == Type.UNTYPED) {
    ...
    public ChatMessage statisticTooltip(final Statistic which, EntityType entity) {
        Type type = which.getType();
        if (type == Type.UNTYPED) {
            throw new IllegalArgumentException("That statistic needs no additional parameter!");
        }
    ...
    public ChatMessage itemTooltip(final String itemJSON) {
        onHover("show_item", itemJSON);
        return this;
    }

    public ChatMessage itemTooltip(final ItemStack itemStack) {
        try {
    ...
    public ChatMessage tooltip(final List<String> lines) {
        return tooltip((String[])lines.toArray());
    }

    public ChatMessage tooltip(final String... lines) {
        if (lines.length == 1) {
    ...
    public ChatMessage then(final Object obj) {
        if (!latest().hasText()) {
    ...
    public void send(Player player){
        ...
        } catch (NullPointerException | InstantiationException | InvocationTargetException | IllegalAccessException | NoSuchMethodException e) {
            e.printStackTrace();
        }
    }
